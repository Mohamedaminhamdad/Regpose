"""
Author: https://github.com/LZGMatrix/CDPN_ICCV2019_ZhigangLi
Modified by Mohamed: 
"""
...
class Evaluation(object):
    def __init__(self, models,classes,model_info):
        self.models = models
        self.pose_est_all = {}
        self.pose_gt_all = {}
        self.quaternion_est_all={}
        self.quaternion_gt_all={}
        self.translation_gt={}
        self.translation_est={}
        self.camera_k={}
        self.num = {}
        self.numAll = 0.
        ...
        for cls in self.classes:
            ...
            self.quaternion_est_all[cls]=[]
            self.quaternion_gt_all[cls]=[]
            self.translation_est[cls]=[]
            self.translation_gt[cls]=[]
            self.pose_gt_all[cls] = []
            self.camera_k[cls]=[]
    def te3d(self):
        logger.info('\n* {} *\n {:^}\n* {} *'.format('-' * 100, 'Evaluation 3D Trans', '-' * 100))

        translation_est = self.translation_est
        translation_gt = self.translation_gt
        trans_errors = []
        cls_trans_errors = {}
        results=[]
        avg_trans_errors = {}
        for cls in self.classes:
            cls_trans_errors[cls] = []
            cls_trans_est = translation_est[cls]
            cls_trans_gt = translation_gt[cls]
            for tran_est, tran_gt in zip(cls_trans_est, cls_trans_gt):
                tran_est=tran_est*100
                tran_gt=tran_gt*100
                error=te(tran_est,tran_gt)
                cls_trans_errors[cls].append(error)
                trans_errors.append(error)
            if len(cls_trans_errors[cls]) != 0:
                avg_error = np.sum(cls_trans_errors[cls]) / len(cls_trans_errors[cls])
                avg_trans_errors[cls] = avg_error
            else:
                avg_trans_errors[cls] = np.nan
            
            logger.info("Class: {} \t\t {}".format(cls, avg_trans_errors[cls]))
            evaluation={"class":cls,"error":avg_error}
            results.append(evaluation)
        total_avg_error = np.sum(trans_errors) / len(trans_errors)
        data={'avg_error':total_avg_error,'single_eror':results}
        with open('translation_error_3d_ego_quat.json', 'w') as outfile:
            json.dump(data, outfile)
        logger.info("All:\t\t\t\t\t {}".format(total_avg_error))
        return 
    def calculate_class_avg_rotation_error(self, output_path):
        ...
        all_quaternions_est = self.quaternion_est_all
        all_quaternions_gt = self.quaternion_gt_all
        rotation_errors = []
        cls_rotation_errors = {}
        results=[]
        avg_rotation_errors = {}
        for cls in self.classes:
            ...
            cls_quaternions_est = all_quaternions_est[cls]
            cls_quaternions_gt = all_quaternions_gt[cls]
            for quaternion_est, quaternion_gt in zip(cls_quaternions_est, cls_quaternions_gt):
                angle_diff = np.degrees(np.arccos(2 * np.square(np.dot(quaternion_gt, quaternion_est)) - 1))
            ...
            logger.info("Class: {} \t\t {}".format(cls, avg_rotation_errors[cls]))
            evaluation={"class":cls,"error":avg_error}
            results.append(evaluation)
        total_avg_error = np.sum(rotation_errors) / len(rotation_errors)
        data={'avg_error':total_avg_error,'single_eror':results}
        with open('rotation_error_quat_allo.json', 'w') as outfile:
            json.dump(data, outfile)
        logger.info("All:\t\t\t\t\t {}".format(total_avg_error))
        return
    def evaluate_pose(self):
        ...
        logger.info('\n* {} *\n {:^}\n* {} *'.format('-' * 100, 'Evaluation 6D Pose', '-' * 100))
        rot_thresh_list = np.arange(0, 200, 10)
        trans_thresh_list = np.arange(0.0, 0.1, 0.005)
        ...
        for i, cls_name in enumerate(self.classes):
            ...
            for thresh_idx in range(num_metric):
                ...
                rotation_speichern.append(rot_acc[i, thresh_idx])
                translation_seichern.append(trans_acc[i, thresh_idx])
                space_speichern.append(space_acc[i, thresh_idx])
            dic={'name':cls_name,'rotation':rotation_speichern,'translation':translation_seichern,'space':space_speichern}
            dictio.append(dic)
        ...
        print(' ')
        speichern={'data':dictio}
        with open('evaluation_trans_rot.json', 'w') as outfile:
            json.dump(speichern, outfile)


    def evaluate_pose_add(self, output_dir, flag=None):
        """
        ...
        total=np.zeros((1,num_thresh), dtype=np.float32)
        ...
        for i, cls_name in enumerate(self.classes):
            ...
            acc_mean = area * 100
            plot_data_inplace.append(acc_mean)
            ...
            total = total+count_correct['mean'][i] / float(count_all[i])
            plot_data[cls_name].append((x_s, y_s))
            plt.plot(x_s, y_s, '-')
            plt.xlim(0, 0.1)
            plt.ylim(0, 1)
            plt.xlabel("Average distance threshold in meter")
            plt.ylabel("accuracy")
            ...
            logger.info('threshold=0.10, correct poses: {}, all poses: {}, accuracy: {:.2f}'.format(
                ...
                acc_010))
            logger.info(" ")
            dictionary={'cls_name':cls_name, 'accuar_0.02':acc_002, 'accuar_0.05':acc_005, 'accuar_0.1':acc_005}
        ...
        logger.info("=" * 30)
        total=total/21
        saving_dict={'quat_net_ego':total.flatten().tolist(),'evaluation_single':data,'mean_acc0.02':sum_acc_mean[iter_i] / num_valid_class,'mean_acc0.05': sum_acc_005[iter_i] / num_valid_class,'mean_acc0.1':sum_acc_010[iter_i] / num_valid_class}
        plot_curve(x_s,total,'Average distance threshold [m]','accuray','{} Metric over 21 Objects'.format(eval_name),eval_name)
        with open('evaluation_quat{}.json'.format(eval_name), 'w') as outfile:
...
def plot_curve(x1,y1,xlabel,ylabel,title,eval_name): 
    params = {
   'axes.labelsize': 8,
   'font.size': 8,
   'legend.fontsize': 10,
   ...
   'figure.figsize': [4.5, 4.5]
    ...
    fig = plt.figure()
    y1=y1.squeeze(0)
    plt.plot(x1, y1, linewidth=2, color='#B22400')
    #legend = plt.legend(["Low mutation rate", "High Mutation rate"], loc=4);
    ...
    plt.grid()
    plt.savefig(os.path.join('Bilder', 'quat_ego_{}.png'.format(eval_name)), dpi=1200)
def RotMat(axis, ang):
    s = np.sin(ang)
    c = np.cos(ang)
    res = np.zeros((3, 3))
...
def calc_all_errs(R_est, T_est, R_gt, T_gt, model_points, K, cls_name):
    ...
    ADI = adi(R_est, T_est, R_gt, T_gt, model_points)
    ADD = add(R_est, T_est, R_gt, T_gt, model_points)

    return err_R, err_T, ARP_2D, ADD, ADI
